...
<head>
  ...
  <link rel="stylesheet" href="./css/ContactArabic.css">
</head>
...
<body style="background-image: url(./image//lease.jpg); background-size: cover;">
    ...
    <div class="container">
      ...
      <div class="box">
        ...
        <div class="name">
          ...
          <input type="text" placeholder="الاسم" name="name" pattern="[A-Za- z\s]+" title="يرجى إدخال أحرف فقط" required autocomplete="off">
        </div>
        <div class="Email">
          <i class="fas fa-envelope"></i>
          <input type="email" placeholder="البريد الالكتروني" name="email" id="email" required autocomplete="off">
        </div>
        <div class='mobile'>
        ...
        <div class="message-box">
          ...
          <textarea cols="28" rows="10" placeholder="ملاحظات" name="message" id="message" required></textarea>
        </div>
        <div class="button">
          <button id="send" type="submit" name="submit">إرسال</button>
        </div>
      </div>
    </div>
  </form>
  ...
  if (isset($_POST['submit'])) {
    ...
    if (preg_match('/\d/', $name)) {
      ...
      header('location:ArabicContact.php');
